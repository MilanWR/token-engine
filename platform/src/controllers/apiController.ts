...
/**
 * @swagger
 * /api/v1/incentive/balance/{accountId}:
 *   get:
 *     summary: Get incentive token balance for an account
 *     tags: [Incentive]
 *     security:
 *       - ApiKeyAuth: []
 *     parameters:
 ...
 *                   example: 100
 */
export const getIncentiveBalance = async (req: Request, res: Response) => {
    try {
        ...
        const balance = await hederaService.getTokenBalance(
            accountId,
            appOwner.tokenIds[0].incentiveTokenId
        );
